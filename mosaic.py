...
def overlay_image(img_path):
    original_image = Image.open(img_path)
    mosaic_img = Image.open(OUT_FILE)

    original_image = original_image.resize((mosaic_img.size[0], mosaic_img.size[1]))

    original_image = original_image.convert("RGBA")
    mosaic_img = mosaic_img.convert("RGBA")

    blended_image = Image.blend(mosaic_img, original_image, alpha=TRANSPARENCY)
    blended_image.save('mosaic_final.png')
    blended_image.show()
